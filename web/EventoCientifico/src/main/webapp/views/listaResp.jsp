...
<html>
<head>
<title>Lista de Responsavel</title>
...
<body>
	...
	<div align="center">
		<h1>Lista de Responsavel </h1>
		<h2>
			<a href="/<%=contextPath%>">Menu Principal</a> &nbsp;&nbsp;&nbsp;
			<%-- <c:if test="${usuario.membroOrganizacao != null}"><a href="/<%= contextPath%>/resp/cadastro">Adicione Novo resp</a></c:if> --%>
		</h2>
	</div>

	<div align="center">
		<table id="tabela">
			<caption>Lista de Responsavel</caption>
			<tr>	
				<th>Primeiro Nome</th>
				<th>Sobrenome</th>

			</tr>
			<c:forEach var="resp" items="${requestScope.listaResp}">
				<%-- <tr <c:if test="${usuarioLogado != null && usuarioLogado.cliente != null}"> onclick="compraPacote(${pacote.id})"</c:if>> --%>
			
						<td>${resp.primeiroNome}</td>
