...
import br.ufscar.dc.pibd.dao.SubmeteTrabalhoDAO;

@WebServlet(urlPatterns = { "/submeteTrabalho/*" }) // e essa url aqui??
public class SubmeteTrabalhoController extends HttpServlet{

	private static final long serialVersionUID = 1L;

    private SubmeteTrabalhoDAO submeteTrabalhoDao;
    
    @Override
    public void init() {
        submeteTrabalhoDao = new SubmeteTrabalhoDAO();
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            request.setAttribute("mensagens", erro);
            request.setAttribute("linkVoltar", "../submeteTrabalho/home");
            RequestDispatcher dispatcher = request.getRequestDispatcher("/views/error.jsp");
            dispatcher.forward(request, response);
    ...
    private Map<Long, String> getSubmeteTrabalho() {
        Map <Int,Int> trabalhosSubmetidos = new HashMap<>();
        for (SubmeteTrabalho submeteTrabalho: new SubmeteTrabalhoDAO().getAll()) {
            trabalhosSubmetidos.put(submeteTrabalho.getIdAutor(), submeteTrabalho.getIdTrabalho());
        }
    ...
    private void apresentaFormCadastro(HttpServletRequest request, HttpServletResponse response)
        ...
        RequestDispatcher dispatcher = request.getRequestDispatcher("/views/formularioSubmeteTrabalho.jsp");
        dispatcher.forward(request, response);
    }
    ...
    private void insere(HttpServletRequest request, HttpServletResponse response)
        ...
        int idTrabalho = request.getParameter("idTrabalho");
        int idAutoria = request.getParameter("idAutoria");
 
        // precisa do id aqui?
        SubmeteTrabalho submeteTrabalho = new SubmeteTrabalho(idTrabalho, idAutoria);
        submeteTrabalhoDao.insert(submeteTrabalho);
    ...
    private void apresentaHome(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        request.setAttribute(getServletName(), response);
        RequestDispatcher dispatcher = request.getRequestDispatcher("/views/homeSubmeteTrabalho.jsp");
        dispatcher.forward(request, response);
    }
